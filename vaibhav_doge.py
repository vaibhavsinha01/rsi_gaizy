...
class DeltaExchangeRFGaizy:
    ...
    def get_active_positions_bitcoin(self):
        """Get active positions from Delta Exchange for DOGEUSD (product_id: 101555)"""
        try:
            method = "GET"
            path = "/v2/positions"
            url = self.base_url + path
    ...
    def get_base_margin_size(self):
        try:
            if hasattr(self,'df'):
                ...
                self.df = pd.read_csv(r"C:\Users\vaibh\OneDrive\Desktop\delta\DOGEUSD_Final.csv")
                self.account_balance = float(self.get_usd_balance())
                self.base_price = float(self.get_market_price()) * self.min_lot
                self.margin = self.account_balance/self.base_price
    ...
    def fetch_data(self):
        try:
            ...
            self.df = self.broker.get_ticker_data(symbol='DOGEUSD') # current time / other things need to be accounted for
            ...
            self.df.to_csv("DOGEUSD_Indicator.csv")
    ...
    def set_leverage_delta(self, value, product_id):
        ...
        Args:
            ...
            product_id (int or str): The Delta Exchange product ID (e.g., 101555 for DOGEUSD).
        """
        import time
        import json
        import requests
    ...
    def execute_signals(self):
        try:
            ...
            self.df.to_csv('DOGEUSD_Final.csv')
            ...
            if last_signal != 0 and self.get_active_positions_bitcoin():
                ...
                if sl_price is not None and tp_price is not None:
                    ...
                    self.set_leverage_delta(value=self.base_leverage,product_id="101555")
                    self.leverage_check()
